...
var HbbTVApp2AppServer = function (httpServer) {
	...
	var handlePendingConnectionsChanged = function (channel) {
		...
		while(channelPendingLocalConnections.length>0 && channelPendingRemoteConnections.length>0){
            ...
            localConnection && (localConnection.readyState == ws.OPEN) && localConnection.send("pairingcomplete");
            remoteConnection && (remoteConnection.readyState == ws.OPEN) && remoteConnection.send("pairingcomplete");
		}
    ...
    var handleConnectionReceived = function(connection) {
        console.log("receive app2app ws connection");
        ...
        if(req.channel){
            ...
            connection.on("message", function(msg, flags) {
                console.log("receive app2app ws message");
                var options = {};
