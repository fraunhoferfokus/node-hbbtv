...
var HbbTVDialServer = function (expressApp, isCsLauncher) {
    ...
    var openUrl = function (url) {
        import('open').then(openModule => {
            try {
                // Use the default export from the open module
                const open = openModule.default;
                // Call open with the new API format if opnParams is provided
                if (opnParams && Array.isArray(opnParams) && opnParams.length > 0) {
                    open(url, {
                        app: {
                            name: opnParams[0],
                            arguments: opnParams.slice(1).join(' ')
                        }
                    });
                } else {
                    open(url);
            ...
            console.error("Error importing open module", err.message);
        });
    };
                ...
                if(app && !app.disabled){
                    if(appName == "HbbTV"){
                        ...
                        if (interDevSyncUrl != null) {
                            app.additionalData["hbbtv:X_HbbTV_InterDevSyncURL"] = String(interDevSyncUrl).replace('{hostname}',this.hostname);
                        }
